/**
 * @fileoverview 此类表示地图上的一个简单更少资源的覆盖物，从Overlay继承。
 * 主入口类是<a href="symbols/BMapLib.SimpleMarker.html">SimpleMarker</a>，
 * 基于Baidu Map API 1.2。
 *
...
(function () { 
    ...
    var _oSimpleMarkerFragment = null;

    /**
     *@exports SimpleMarker as BMapLib.SimpleMarker
     */
    var SimpleMarker = 
        /**
        * SimpleMarker
        ...
        *@param {Json Object} options 可选参数，可包含以下几个属性：<br />
        *   isLabel {Boolean} 是否放入label图层。(可选)<br />
        *   url	{String} 图片的url地址。(可选)<br />
        ...
        *   text {String}    表示该覆盖物显示的文字信息。(可选)<br />
        *   textSize {Number} 文字的大小。（可选，默认10）<br />
        *   textColor {String} 文字的颜色。（可选，默认black）<br />
        *   batchMode {Boolean} 是否启用批量处理模式。(可选，默认false)<br />
        *   style {String} 其他样式
        */
        BMapLib.SimpleMarker = function(position, options) {
            this._position = position;
            this._options = options || {};
            this._isLabel = this._options['isLabel'] || false;
            this._text = this._options['text'] || false;
            this._batchMode = this._options['batchMode'] || false;

            this._style = {};
            // this._style['url'] = this._options['url'] || _IMAGE_PATH + '0.' + _IMAGE_EXTENSION;
            this._style['url'] = this._options['url'] || false;
            this._style['size'] = this._options['size'] || new BMap.Size(53, 53);
            this._style['anchor'] = this._options['anchor'];
            this._style['offset'] = this._options['offset'];
            this._style['textSize'] = this._options['textSize'];
            this._style['textColor'] = this._options['textColor'];
            this._style['style'] = this._options['style'] || false;
        };

    /**
    *批量模式时需要最后调用此方法显示
    *@param {Map} map BMap.Map的实例化对象。
    *@param {Boolean} isLabel 是否放入label图层。(可选)<br />
    */
    SimpleMarker.show = function(map, isLabel) {
        if (_oSimpleMarkerFragment != null) {
            if (isLabel)
                map.getPanes().labelPane.appendChild(_oSimpleMarkerFragment);
            else
                map.getPanes().markerMouseTarget.appendChild(_oSimpleMarkerFragment);
            _oSimpleMarkerFragment = null;
        }
    }

    T.lang.inherits(SimpleMarker, BMap.Overlay, "SimpleMarker");
    ...
    SimpleMarker.prototype.initialize = function(map) {
        ...
        if (!this._domElement) {
            if (this._isLabel) {
                ...
                this._domElement.className = "BMap_SimpleLabel";
            } else {
                this._domElement = document.createElement('div');
                this._domElement.className = "BMap_SimpleMarker";
            }
            this._updateCss();    
            this._updateText();
            this._bind();
        ...
        if (this._batchMode) {
            // 批处理模式采用createDocumentFragment提高性能
            if (_oSimpleMarkerFragment == null)
                _oSimpleMarkerFragment = document.createDocumentFragment();
            _oSimpleMarkerFragment.appendChild(this._domElement);
        } else {
    ...
    */
    SimpleMarker.prototype.draw = function(){
        // TODO 增加这个判断后addOverlay会触发导致批量模式失效
        // if (!this.isDrawed()) {
        //     if (this._isLabel)
    ...
    /**
    * 获取该覆盖物是否已经绘制。
    *@return {Boolean} 是否已经绘制。
    */
    SimpleMarker.prototype.isDrawed = function() {
        var parent = this._domElement.parentNode;
        return parent && parent.tagName && parent.tagName == 'DIV';
    }
    ...
    */
    SimpleMarker.prototype.getText = function(){
        return this._text;
    };
    ...
    *@return 无返回值。
    */
    SimpleMarker.prototype.setImage = function(url){
        var _url = this._style['url'];
        if(url && (!_url || (_url.toString() != url.toString()))){
            this._style['url'] = url;
            this._updateImage(); 
        }
    };
    ...
    /**
    *设置该覆盖物的背景图相对于可视区域的偏移值。
    *@param {Size} offset 图片相对于可视区域的偏移值，此功能的作用等同于CSS中的background-position属性。
    *@return 无返回值。
    */
    SimpleMarker.prototype.setOffset = function(offset){
        if (offset && offset.length)
            offset = new BMap.Size(offset[0], offset[1]); 
        var _offset = this._style['offset'];
        if(offset && (!_offset || _offset.width != offset.width || _offset.height != offset.height)){
            this._style['offset'] = offset;
            this._updateOffset(); 
        }
    };
    ...
    *@return {String}。
    */
    SimpleMarker.prototype.getImage = function(){
        return this._style['url'] || '';
    };
    ...
    */
    ...
    /**
    *获取该覆盖物的位置。
    *@return {Pixel} 该覆盖物的屏幕坐标。
    */
    SimpleMarker.prototype.getPixel = function() {
        if(!this._map) return;
        if('undefined' == typeof this._pixelPosition || !this.isDrawed()) {
            // log.debug('========');
            // log.debug(this._map);
    ...
    *@return 无返回值。
    */
    SimpleMarker.prototype.openInfoWindow = function(InfoWindow) {
        if(!InfoWindow) return;
        return this._map.openInfoWindow(InfoWindow,this._position);
    };
    ...
    *@return 无返回值。
    */
    SimpleMarker.prototype._updateCss = function(){
    ...
    *@return 无返回值。
    */
    SimpleMarker.prototype._updateText = function(){
        if (this._domElement && this._text && this._text != this._domElement.innerHTML) {
    ...
    *@return 无返回值。
    */
    SimpleMarker.prototype._updateImage = function(){
        var _url = this._style['url'];
        if (this._domElement && _url) {
    ...
    *@return 无返回值。
    */
    SimpleMarker.prototype._updateOffset = function(){
        var _offset = this._style['offset'];
        if (this._domElement && _offset) {
            var style = this._domElement.style;
            style.backgroundPosition = _offset.width + 'px ' + _offset.height + 'px';
        }
    };
    ...
    *@return 无返回值。
    */
    SimpleMarker.prototype._updatePosition = function(){
        if (this._domElement && this._position) {
            var style = this._domElement.style;
            var pixelPosition= this._map.pointToOverlayPixel(this._position); 
            this._pixelPosition = pixelPosition;

            var anchor = this._style['anchor'];
            if (anchor instanceof BMap.Size) {
                pixelPosition.x -= anchor.width;
                pixelPosition.y -= anchor.height;
            } else {
                // pixelPosition.x -= Math.ceil(parseInt(style.width) / 2);
                // pixelPosition.y -= Math.ceil(parseInt(style.height) / 2);
            }

            var left = pixelPosition.x + "px";
            if (style.left != left) style.left = left;
            var top = pixelPosition.y + "px";
    ...
    */
    SimpleMarker.prototype._buildCssText = function(style) {
        //根据style来确定一些默认值
        var url = style['url'];
        var size = style['size'];
        var offset = style['offset'];
        var cstyle = style['style'];

        var csstext = [];
        if (T.browser["ie"] < 7) {
            if(url) csstext.push('filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(' +
                'sizingMethod=scale,src="' + url + '");');
        } else {
            if(url) csstext.push('background-image:url(' + url + ');background-repeat:no-repeat;');
        ...
        if(cstyle) {
            if(cstyle instanceof Object) {
                var newstyle = '', comma = '';
                for(var key in cstyle) {
                    newstyle += comma + key + ':' + cstyle[key];
                    comma = '; ';
                }
                csstext.push(newstyle);
            } else {
                csstext.push(cstyle);
            }
        }
        ...
        if (this._text) {
            ...
            csstext.push('color:' + textColor + '; font-size:' + textSize + 'px; font-family:Arial,sans-serif; font-weight:bold');
        }
        return csstext.join('');
    };
     ...
     * <br />"<b>target</b>：{BMapLib.SimpleMarker} 事件目标
     ...
     * mySimpleMarker.addEventListener("mouseover", function(e) {  alert(e.point);  });
     ...
     * mySimpleMarker.addEventListener("mouseout", function(e) {  alert(e.point);  });
    ...
    * @return 无返回值。
    */
    SimpleMarker.prototype._bind = function(){
        if (!this._domElement){
            return;
        }
