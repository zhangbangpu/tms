...
if(typeof parms.id != 'undefined' && parms.id != '') {
	var entity = $ips.load("resource", "get", "id=" + parms["id"]);
	if (entity) {
		if (entity.subsystem) {
			renderParentSelect2(entity.subsystem);
			$("#parentid").select2('val', entity.parentid);
		}
...
function resourceSave(newed) {
	...
	pararm.push({'name' : 'parentid', 'value' : $('#parentid').select2('val')});
	if(isupdate)
		pararm['id'] = updateid;
	$ips.load("resource", "save", pararm, function(result){
		if(result) {
			$ips.succeed("保存成功。");
			if (newed) {
				isupdate = updateid = 0;
...
function renderParentSelect2(subsystem)
{
	...
	if (subsystem != '') {
		var res = $ips.load('resource', 'parentcompletion', {subsystem : subsystem});
		if (res) {
			$.each(res, function () {
				var name = this.name;
				var pre = '';
...
$ips.load('resource', 'getAllResource', {}, function (data) {
